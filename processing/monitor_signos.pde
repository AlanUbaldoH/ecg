...
int ecs_rx_state = 0;
int CES_Pkt_Len;
int CES_Pkt_Pos_Counter, CES_Pkt_Data_Counter1, CES_Pkt_Data_Counter2,CES_Pkt_Data_Counter3,CES_Pkt_Data_Counter4;
int CES_Pkt_PktType;
char DataRcvPacket1[] = new char[500];
char DataRcvPacket2[] = new char[500];
char DataRcvPacket3[] = new char[500];
char DataRcvPacket4[] = new char[500];
...
double receivedVoltage_RED, receivedVoltage_IR,receivedVoltage_ECG ,receivedVoltage_T;
BigDecimal min, max;
...
float[] Avgecgdata = new float[pSize];
float tempdata= 0;
float[] Avgtempdata = new float[pSize];
int arrayIndex = 1;
Graph g, g1;
float time =0;
...
public void draw() {
 ...
 // msgBox.draw();
  headerButton.draw();
  helpWidget.draw();
  fill(255);                           // Heading Title
    textAlign(CENTER);
    textSize(30);
    text("Temperatura: "+tempdata, 800, 100);
}
...
void ecsProcessData(char rxch)
{
  ...
  case CESState_SOF2_Found:
    ...
    CES_Pkt_Data_Counter3 = 0;
    CES_Pkt_Data_Counter4 = 0;
    break;
  ...
  case CESState_PktLen_Found:
    ...
    } else if ( (CES_Pkt_Pos_Counter >= CES_CMDIF_PKT_OVERHEAD) && (CES_Pkt_Pos_Counter < CES_CMDIF_PKT_OVERHEAD+CES_Pkt_Len+1) )  //Read Data
    {
      ...
      if (CES_Pkt_PktType == 2)
      {
        ...
        } else if(CES_Pkt_Data_Counter1 >= 4 && CES_Pkt_Data_Counter2 < 4)
        {
          DataRcvPacket2[CES_Pkt_Data_Counter2]= (char) (rxch);
          CES_Pkt_Data_Counter2++;
        }else if(CES_Pkt_Data_Counter2 >= 4 && CES_Pkt_Data_Counter3 < 4)
        {
          DataRcvPacket3[CES_Pkt_Data_Counter3]= (char) (rxch);
          CES_Pkt_Data_Counter3++;
          
        }
        else if(CES_Pkt_Data_Counter3 >= 4 && CES_Pkt_Data_Counter4 < 4)
        {
          DataRcvPacket4[CES_Pkt_Data_Counter4]= (char) (rxch);
          CES_Pkt_Data_Counter4++;
        }
    ...
    } else  //All header and data received
    {
      ...
      {     
        ...
        int data3 = ecsParsePacket(DataRcvPacket3, DataRcvPacket3.length-1);
        int data4 = ecsParsePacket(DataRcvPacket4, DataRcvPacket4.length-1);
        receivedVoltage_RED = data1  ;
        receivedVoltage_IR = data2  ;
        receivedVoltage_ECG = data3  ;
        ...
        Avgecgdata[arrayIndex] = (float)receivedVoltage_ECG;
        Avgtempdata[arrayIndex] = (float)receivedVoltage_T;
        value1 = (float)( AvgYdata[arrayIndex] - averageValue(AvgYdata));
        value2 = (float)( AvgZdata[arrayIndex] - averageValue(AvgZdata));
        value3 = (float)( Avgecgdata[arrayIndex] - averageValue(Avgecgdata));
       
        ydata[arrayIndex] = value1;
        zdata[arrayIndex] = value2;
        ecgdata[arrayIndex] = value3;
        tempdata =(float) data4;
        ...
        {  
          ...
          SpO2 = (int)(SpO2 * 100);
          SpO2 = SpO2/100;
          oxygenSaturation.setText("SpO2: "+SpO2+"");
        }
        if (startPlot) {
        }
        ...
        maxAxis_ir = max(ecgdata);
        minAxis_ir = min(ecgdata);
